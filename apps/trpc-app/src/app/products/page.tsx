...
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from '../_components/landing/ui/select';
...
export default function ProductsPage() {
  ...
  // Derived filtered products based on search query & region (demo purposes)
  const filteredProducts = SAMPLE_PRODUCTS.filter(p => {
    ...
    if (region !== 'all') {
      matches = matches && p.region.toLowerCase() === region.toLowerCase();
    }
    // Note: categories & other filters can be added here later
    return matches;
  });
  ...
  const NotFound: React.FC = () => (
    <div className='flex flex-col items-center justify-center text-center py-24'>
      ...
      {/*<p className='mt-2 text-xs text-gray-500'>or adjust your search criteria</p> */}
    </div>
  );
  ...
  // sync categories param on changes
  ...
  // Effect to update URL param when selectedCats changes
  useEffect(() => {
